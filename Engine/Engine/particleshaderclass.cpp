...
bool ParticleShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	result = D3DX11CompileFromFile(vsFilename, NULL, NULL, "ParticleVertexShader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, NULL, &vertexShaderBuffer, &errorMessage, NULL);
	if (FAILED(result))
	{
		if (errorMessage)
		{
	...
	result = D3DX11CompileFromFile(psFilename, NULL, NULL, "ParticlePixelShader", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, NULL, &pixelShaderBuffer, &errorMessage, NULL);
	if (FAILED(result))
	{
		if (errorMessage)
		{
	...
	//Create the vertex input layout description - the third component is simply the color of each particle
	...
	//Countthe elements in the layout
	...
	D3DXMATRIX projectionMatrix, ID3D11ShaderResourceView* texture)
{
	...
	//Transpose the matrices to prepare them for the shader for memory alignment purposes
	...
	//Copy the matrices ino the constant buffer
	...
	//Set shader texture resource in teh pixel shader

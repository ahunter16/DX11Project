...
bool TextClass::SetFps(int fps, ID3D11DeviceContext* deviceContext)
{
	...
	//truncate teh fps to below 10,000
	if (fps > 9999)
	{
		fps = 9999;
	}
	...
	//If fps is below 30, set fps color to red
	if (fps < 30)
	{
		red = 1.0f;
		green = 0.0f;
	...
	//Update the sentence vertex buffer with the new string information
	result = UpdateSentence(m_sentence3, fpsString, m_screenWidth - 60, 20, red, green, blue, deviceContext);
	if (!result)
	{
		return false;
	}
...
bool TextClass::SetCpu(int cpu, ID3D11DeviceContext* deviceContext)
{
	...
	//Update the sentence vertex buffer with the new string information
	result = UpdateSentence(m_sentence4, cpuString, m_screenWidth - 60, 40, 0.0f, 1.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}
...
bool TextClass::SetRenderCount(int renderCount, ID3D11DeviceContext* deviceContext)
{
	...
	//Setup the cpu string
	strcpy_s(renderString, "Rendered: ");
	strcat_s(renderString, tempString);
	

	//Update the sentence vertex buffer with the new string information
	result = UpdateSentence(m_sentence5, renderString, m_screenWidth/2, 20, 0.0f, 1.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}

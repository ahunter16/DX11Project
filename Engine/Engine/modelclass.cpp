...
bool ModelClass::Initialize(ID3D11Device* device, char* modelFilename, WCHAR* textureFilename1, WCHAR* textureFilename2, HWND hwnd)
{
	bool result;
	...
	if(!result)
	{
		MessageBox(hwnd, L"Could not initialize buffers.", L"Error", MB_OK);
		return false;
	}
	...
	if (!result)
	{
		MessageBox(hwnd, L"Could not initialize textures.", L"Error", MB_OK);
		return false;
	}
